...
- Requires special syntax for operators description

## Operators sets

This conception might be not popular, as I've never seen an idea like that. Anyway, for me, it sounds not really weird. 

The idea is not to allow creating custom operators, but instead allow to use one of predefined. These predefined operators are not common and should not be used in std, instead, the user is free to use them in a library.
...
This is the specification proposed for the second way -- fully customizable operators. Operator sets do not need any specification as they just use a common way with traits and implementations. 
...
I see two ways to solve the problem of lack of information about operators on the parsing stage:
...
Before name resolution all items are declared, it gives the compiler an opportunity to resolve items that haven't appear in code so far.
...
`operator type` pair is used to reduce the count of keywords against Swift solution with the `precedencegroup` keyword.
`operator` might be a soft keyword in the future, anyway as all of the current keywords now, it gonna be a hard keyword.
...
- `or`/`and` - These logic operators are "short-circuit" operators, thus cannot be overloaded without lazy evaluation (no lazy in _Jacy_ so far)
- `...` - The spread operator is not a real operator (maybe it will be changed in the future), it is mostly punctuation with different behavior depending on context.
...
Trait operators are overloadable operators with syntax inexpressible as a prefix, infix, or postfix operators, e.g. `()` for invocations. Of course, we would allow writing something like `func ()() {}`, but we've got a rule: "All operators that written as identifiers (such as in `func +` where `+` is an ident-like) -- are customized if not reserved", thus there're no special cases like "you can overload `func ()()` but cannot define `postfix operator ()`".
...
- `Drop` - not a real operator, anyway it is included in the `std::ops` module.
...
I don't want to discuss the generality of Rust `?` operator, instead I want to bring up the subject of having both functionalities.
...
Keeping this in mind I would say that `?` should mean the optional chaining.

> SOLUTION: `?` stands for "optional-chaining"

__Question 3.__

In question 2. I said that would like to see `?` as an optional-chaining operator, so now I need to choose an operator for error-propagation. It must be a non-conflicting postfix operator with obvious meaning.
...
Let's at first describe what symbols can operators contain, start/end with, as these rules are needed to avoid breaking the entire language syntax.
...
Although operators cannot contain `:`, there's one special case -- `:=` operator which is overloadable.
...
Anyway, I already proposed a syntax where function generics are written after the `func` keyword (like in Kotlin) instead of after identifier.
