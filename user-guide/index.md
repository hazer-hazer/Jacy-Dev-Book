...
_Jacy_ might be easy to read for users familiar with C-like syntax. Its syntax is influenced by Rust, Swift and C++, that are C-like language though.
...
Identifiers in _Jacy_ start with any english letter or `_` (underscore) followed by `_`, letters or digits.
...
There're also reserved keywords which are unused: `do`, `import`, `macro`, `of`.

### Operators

Operators are special symbols which you use to work with values.
There're three kinds of operators: prefix (unary), infix (binary) and postfix (unary).
Prefix, infix and postfix are about where operator is placed: before, between or after expression(s).
...
Most of infix operators are left-associative.

Prefix and postfix operators in _Jacy_ all have one precedence by groups: postfix is stronger than prefix operator.

#### Operator precedence table

This table shows which operators are stronger than other, associativity is marked as "left to right" for left-associative operators and "right to left" for right-associative operators.
This table also includes expressions which are not actually operator-expressions, anyway, it might be helpful to know that they are parsed assuming this figurative precedence.

> Precedence index is placed just for some help, sometimes you need to know the order.

Table is from high to low precedence ordered -- the operators in the first row have the strongest precedence.
